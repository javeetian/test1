...
bool CoogleIOT::initialize()
{
	...
	info("\r\nldos IOT v" COOGLEIOT_VERSION " initializing..");
...
/*
	if (!connectToUDP())
	{
		error("Failed to connect to UDP");
	}

	if (!connectToTCP())
	{
		error("Failed to connect to TCP");
	}
*/
	return true;
}
...
int CoogleIOT::getBaudrate()
{
	...
	if (val == 115200 || val == 38400 || val == 9600)
		return val;
	else
		return 115200;
}
...
int CoogleIOT::getWiFiMode()
{
	...
	if (val == WIFI_MODE_AP || val == WIFI_MODE_STATION || val == WIFI_MODE_AP_STATION)
		return val;
	else
		return WIFI_MODE_AP;
}

CoogleIOT &CoogleIOT::setLocalIP(int val)
{

	if (!eeprom.writeInt(WIFI_LOCAL_IP_ADDR_START, val))
	{
		error("Failed to write local IP to EEPROM");
	}

	return *this;
}

int CoogleIOT::getLocalIP()
{
	int val;
	if (!eeprom.readInt(WIFI_LOCAL_IP_ADDR_START, &val))
	{
		error("Failed to read local IP from EEPROM");
	}
	if (val == 0 || val == 0xffffffff)
		return (1 << 24) + (0 << 16) + (168 << 8) + 192;
	else
		return val;
}

CoogleIOT &CoogleIOT::setLocalMask(int val)
{

	if (!eeprom.writeInt(WIFI_LOCAL_MASK_ADDR_START, val))
	{
		error("Failed to write local mask to EEPROM");
	}

	return *this;
}

int CoogleIOT::getLocalMask()
{
	int val;
	if (!eeprom.readInt(WIFI_LOCAL_MASK_ADDR_START, &val))
	{
		error("Failed to read local mask from EEPROM");
	}
	if (val == 0 || val == 0xffffffff)
...
CoogleIOT &CoogleIOT::setLocalGateway(int val)
{

	if (!eeprom.writeInt(WIFI_LOCAL_GATEWAY_ADDR_START, val))
	{
		error("Failed to write local gateway to EEPROM");
	}

	return *this;
}

int CoogleIOT::getLocalGateway()
{
	int val;
	if (!eeprom.readInt(WIFI_LOCAL_GATEWAY_ADDR_START, &val))
	{
		error("Failed to read local gateway from EEPROM");
	}
	if (val == 0 || val == 0xffffffff)
		return (1 << 24) + (0 << 16) + (168 << 8) + 192;
	else
...
int CoogleIOT::getTCPMode()
{
	...
	if (val == TCP_MODE_CLIENT || val == TCP_MODE_SERVER)
		return val;
	else
		return TCP_MODE_SERVER;
}

CoogleIOT &CoogleIOT::setTCPLocalIP(int val)
{

	if (!eeprom.writeInt(TCP_LOCAL_IP_ADDR_START, val))
	{
		error("Failed to write local tcp ip to EEPROM");
	}

	return *this;
}

int CoogleIOT::getTCPLocalIP()
{
	int val;
	if (!eeprom.readInt(TCP_LOCAL_IP_ADDR_START, &val))
	{
		error("Failed to read local tcp ip from EEPROM");
	}
	if (val == 0 || val == 0xffffffff)
		return (1 << 24) + (0 << 16) + (168 << 8) + 192;
	else
...
CoogleIOT &CoogleIOT::setTCPLocalPort(int val)
{

	if (!eeprom.writeInt(TCP_LOCAL_PORT_ADDR_START, val))
	{
		error("Failed to write TCP local port to EEPROM");
	}

	return *this;
}

int CoogleIOT::getTCPLocalPort()
{
	int val;
	if (!eeprom.readInt(TCP_LOCAL_PORT_ADDR_START, &val))
	{
		error("Failed to read TCP local port from EEPROM");
	}
...
int CoogleIOT::getUDPMode()
{
	...
	if (val == UDP_MODE_CLIENT || val == UDP_MODE_SERVER)
		return val;
	else
		return UDP_MODE_SERVER;
}

CoogleIOT &CoogleIOT::setUDPLocalIP(int val)
{

	if (!eeprom.writeInt(UDP_LOCAL_IP_ADDR_START, val))
	{
...
int CoogleIOT::getUDPLocalIP()
{
	int val;
	if (!eeprom.readInt(UDP_LOCAL_IP_ADDR_START, &val))
	{
...
bool CoogleIOT::connectToWiFi()
{
	...
	int lIP = getLocalIP();
	int lMask = getLocalMask();
	int lGateway = getLocalGateway();
	...
	IPAddress apLocalIP(lIP);
	IPAddress apSubnetMask(lMask);
	IPAddress apGateway(lGateway);
	...
	if (lPassword.length() == 0)
	{
		...
		lPassword = COOGLEIOT_AP_DEFAULT_PASSWORD;
		setWiFiLocalPassword(lPassword);
	}
	...
	if (lSSID.length() == 0)
	{
		...
		lSSID = COOGLEIOT_AP;
		lSSID.concat((int)random(100000, 999999));

		info("Setting AP Name To: ");
		info(lSSID);

		setWiFiLocalSSID(lSSID);
	}
